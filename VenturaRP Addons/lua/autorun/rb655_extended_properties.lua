...
SyncFuncs.prop_door_rotating = function( ent )
	...
	ent:SetNWBool( "Closed", state == 0 || state == 3 )
end
...
AddEntFireProperty( "rb655_turret_ammo_remove", "Deplete Ammo", 656, function( ent )
	if ( bit.band( ent:GetSpawnFlags(), 256 ) == 256 ) then return false end
	if ( ent:GetClass() == "npc_turret_floor" ) then return true end
	return false
end, "DepleteAmmo", "icon16/delete.png")
AddEntFireProperty( "rb655_turret_ammo_restore", "Restore Ammo", 657, function( ent )
	if ( bit.band( ent:GetSpawnFlags(), 256 ) == 0 ) then return false end
	if ( ent:GetClass() == "npc_turret_floor" ) then return true end
	return false
end, "RestoreAmmo", "icon16/add.png" )
...
AddEntFireProperty("rb655_alyx_HolsterWeapon", "Holster Weapon", 655, function( ent )
	if ( !ent:IsNPC() || ent:GetClass() != "npc_alyx" || !IsValid( ent:GetActiveWeapon() ) ) then return false end
	return true
...
// This whole thing is a huge hack - not proud of it one bit
local passive = {
	"npc_seagull", "npc_crow", "npc_piegon",
	"npc_cscanner", "npc_clawscanner", "npc_turret_floor",
	"npc_dog", "npc_gman", "npc_antlion_grub", "npc_breen",
	"npc_stalker",
	"monster_cockroach"
}
...
hook.Add( "OnEntityCreated", "rb655_properties_friently/hostile", function( ent )
	if ( ent:IsNPC() ) then
		if ( table.HasValue( friendly, ent:GetClass() ) ) then
			for id, fnpc in pairs( friendliedNPCs ) do
				if ( !IsValid( fnpc ) ) then table.remove( friendliedNPCs, id ) continue end
				fnpc:AddEntityRelationship( ent, D_LI, 999 )
				ent:AddEntityRelationship( fnpc, D_LI, 999 )
			end
			for id, fnpc in pairs( hostaliziedNPCs ) do
				if ( !IsValid( fnpc ) ) then table.remove( hostaliziedNPCs, id ) continue end
				fnpc:AddEntityRelationship( ent, D_HT, 999 )
				ent:AddEntityRelationship( fnpc, D_HT, 999 )
			end
		elseif ( table.HasValue( hostile, ent:GetClass() ) ) then
			for id, fnpc in pairs( friendliedNPCs ) do
				if ( !IsValid( fnpc ) ) then table.remove( friendliedNPCs, id ) continue end
				fnpc:AddEntityRelationship( ent, D_HT, 999 )
				ent:AddEntityRelationship( fnpc, D_HT, 999 )
		...
		elseif ( table.HasValue( monsters, ent:GetClass() ) ) then
			for id, fnpc in pairs( friendliedNPCs ) do
				if ( !IsValid( fnpc ) ) then table.remove( friendliedNPCs, id ) continue end
				fnpc:AddEntityRelationship( ent, D_HT, 999 )
				ent:AddEntityRelationship( fnpc, D_HT, 999 )
...
AddEntFunctionProperty( "rb655_vehicle_exit", "Kick Driver", 655, function( ent )
	...
	if ( !IsValid( ent:GetDriver() ) || !ent:GetDriver().ExitVehicle ) then return end
	ent:GetDriver():ExitVehicle()
end, "icon16/car.png" )

AddEntFireProperty( "rb655_vehicle_radar", "Enable Radar", 655, function( ent )
	if ( !ent:IsVehicle() || ent:GetClass() != "prop_vehicle_jeep" ) then return false end
...
AddEntFireProperty( "func_rotating_forward", "Start Forward", 655, "func_rotating", "StartForward", "icon16/arrow_right.png")
AddEntFireProperty( "func_rotating_backward", "Start Backward", 656, "func_rotating", "StartBackward", "icon16/arrow_left.png")
AddEntFireProperty( "func_rotating_reverse", "Reverse", 657, "func_rotating", "Reverse", "icon16/arrow_undo.png")
AddEntFireProperty( "func_rotating_stop", "Stop", 658, "func_rotating", "Stop", "icon16/shape_square.png")

AddEntFireProperty( "func_platrot_up", "Go Up", 655, "func_platrot", "GoUp", "icon16/arrow_up.png")
AddEntFireProperty( "func_platrot_down", "Go Down", 656, "func_platrot", "GoDown", "icon16/arrow_down.png")
AddEntFireProperty( "func_platrot_toggle", "Toggle", 657, "func_platrot", "Toggle", ToggleIcon)

AddEntFireProperty( "func_train_start", "Start", 655, "func_train", "Start", "icon16/arrow_right.png" )
AddEntFireProperty( "func_train_stop", "Stop", 656, "func_train", "Stop", "icon16/arrow_left.png")
AddEntFireProperty( "func_train_toggle", "Toggle", 657, "func_train", "Toggle", ToggleIcon)

-------------------------------------------------- Pickupable Items --------------------------------------------------

AddEntFunctionProperty( "item_suit", "Wear", 655, function( ent, ply )
...
AddEntFunctionProperty( "pickupitem", "Pick up", 655, function( ent, ply )
